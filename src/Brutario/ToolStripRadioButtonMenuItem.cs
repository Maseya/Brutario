namespace Brutario
{
    ...
    public class ToolStripRadioButtonMenuItem : ToolStripMenuItem
    {
        ...
        // Enable the item only if its parent item is in the checked state and its
        // Enabled property has not been explicitly set to false.
        public override bool Enabled
        {
            get
            {
                // Use the base value in design mode to prevent the designer from
                // setting the base value to the calculated value.
                return base.Enabled && (
                    DesignMode ||
        ...
        protected override void OnCheckedChanged(EventArgs e)
        {
            ...
            // If this item is no longer in the checked state or if its parent has not
            // yet been initialized, do nothing.
            ...
            foreach (ToolStripItem item in Parent.Items)
            {
                    ...
                    && radioItem.Checked)
                {
                    ...
                    // Only one item can be selected at a time, so there is no need to
                    // continue.
                    return;
                }
            }
        ...
        {
            // If the item is already in the checked state, do not call the base
            // method, which would toggle the value.
            if (Checked)
                return;
        ...
        // Let the item paint itself, and then paint the RadioButton where the check
        // mark is normally displayed.
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            {
                // If the Image property is not set, call the base OnPaint method with
                // the CheckState property temporarily cleared to prevent the check
                // mark from being painted.
        ...
        // When the checked state of the parent item changes, repaint the item so that
        // the new Enabled state is displayed.
